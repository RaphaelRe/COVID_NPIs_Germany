...
def calculate_EHt(cases, parameter_values, country, hosp_change_index):
    EH_t = np.zeros(cases.shape)
    EH_t = calculate_EHt_fast(EH_t, cases=cases, xi_H=parameter_values['xi_H'],
                              pi_H=parameter_values['piH']['piH_' + country],
                              hosp_change=parameter_values['hosp_change'] * hosp_change_index) * \
           parameter_values['correction_hospitalization_' + country]
    return(EH_t)


def calculate_EHicut(cases, parameter_values, country, icu_change_index):
    EH_t = np.zeros(cases.shape)
    EH_t = calculate_EHt_fast(EH_t, cases=cases, xi_H=parameter_values['xi_Hicu'],
                              pi_H=parameter_values['piHicu']['piHicu_' + country],
                              hosp_change=parameter_values['icu_change'] * icu_change_index) * \
           parameter_values['correction_hospitalization_' + country]
    return(EH_t)


@jit(nopython=True)
def calculate_EHt_fast(EH_t, cases, xi_H, pi_H, hosp_change):
    for t in range(len(cases)):
        sum_u_t = 0
        for u in range(t + 1):
            sum_u_t += cases[u] * xi_H[t - u]
        EH_t[t] = sum_u_t * pi_H
    return(EH_t * (1 - hosp_change))
