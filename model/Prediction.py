...
class Prediction:
    ...
    def __init__(self, model, data, nb_futue_values,
        ...
        for country_iter in self.countries:
            for ind in range(nb_futue_values):
                ...
                elif 'hosp_change_index' in self.data[country_iter].columns:
                    new_hosp_change = self.data[country_iter].hosp_change_index.iloc[-1]
                    new_icu_change = self.data[country_iter].icu_change_index.iloc[-1]
                    self.data[country_iter] = self.data[country_iter].append({
                    'date': new_date,
                    'weekday': new_weekday,
                    'hosp_change_index': new_hosp_change,
                    'icu_change_index': new_icu_change,
                    },ignore_index=True)
                else:
                    self.data[country_iter] = self.data[country_iter].append({
                    'date': new_date,
    ...
    def predict_observed_quantities(self, parameter_values, latent_variables):
        ...
        for country_iter in self.countries:
            ...
            EH_t = basics.calculate_EHt(cases, parameter_values_tmp, country_iter, hosp_change_index)
            EHicu_t = basics.calculate_EHicut(cases, parameter_values_tmp, country_iter, icu_change_index)
